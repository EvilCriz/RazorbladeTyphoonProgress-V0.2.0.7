...
public class ProjectileRazorbladeTyphoonProgress : ModProjectile
{
    ...
    private CustCl.EffectsSystem EffectsSystem = CustCl.EffectsSystem.GetOrRegister();
    ...
    public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
    {
		...
		//[RU]: (В данном случае мы можем использовать player, т.к. этот код вызовется только у владельца снаряда)
		...
		//[EN]: (In this case, we can use player, as this code will only be called for the projectile owner)
		//[EN]: (Thanks to the previous check)
		//[EN]: The PlayerSave class contains all data about the current level, damage dealt, etc. for each category.
		//[EN]: Exception: The maximum level of the "Additional Effects" category is stored in the "EffectsSystem" class.
		//[EN]: See Common/Player/PlayerSave.cs
        Common.PlayerSave ps = player.GetModPlayer<Common.PlayerSave>();
        ...
        int level = ps.PlayerLevelEffect <= EffectsSystem.MaxLevel ? ps.PlayerLevelEffect : EffectsSystem.MaxLevel;
        ...
        foreach(int i = 0; i < level; i++)
		{
			var lvlInfo = EffectsSystem.LevelsInfo[i];
			
			//[RU]: Проверяем, присвоено ли значение делегату ActionNPC - null
			...
			//[RU]: См. файл CustomClasses/EffectsSystem.cs.
			//[RU]: Дополнительно проверяем, чтобы перебираемый
			//------------------------------------------
			//[EN]: Check if the ActionNPC delegate is set to null
			...
			//[EN]: See the CustomClasses/EffectsSystem.cs file.
			//[RU]: Additionally, we check that the iterated
			if(lvlInfo.ActionNPC is not null)
				lvlInfo.ActionNPC(target);
		}
		...
		foreach(int i = 0; i < level; i++)
			...
			//[EN]: See the CustomClasses/EffectsSystem.cs file
			=> count += EffectsSystem.LevelsInfo[i].CountHealPercent;
		
		//[RU]: Если count > 0 == true - это обозначает, что у игрока открыт 1 и более эффект исцеления (см. выше)
		//[RU]: Если player.statLife < player.statLifeMax2 == true - это обозначает, что у игрока НЕ максимальное здоровье
		//------------------------------------------
		//[EN]: If count > 0 == true - this means that the player has 1 or more healing effects open (see above)
		//[EN]: If player.statLife < player.statLifeMax2 == true - this means that the player does NOT have maximum health
		if(count > 0 && player.statLife < player.statLifeMax2)
		{
			int heal = (int)Math.Ceiling(damageDone / 100f * count);
			player.Heal(heal);
		}
